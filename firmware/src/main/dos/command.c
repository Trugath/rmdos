...
void do_type(void)
{
    int h;
    int n;
    if (!next_token(arg1, sizeof(arg1))) {
        h = 0;
    } else {
        h = dos_open(arg1, 0);
        if (h == -1) {
    ...
    n = dos_read(h, copybuf, sizeof(copybuf));
    while (n > 0) {
...
void do_del(void)
{
    ...
    dos_set_dta(dta);
    if (dos_find_first(arg1, FA_NORMAL) == -1) {
        print_dollar("DEL file\r\n$");
        last_errorlevel = 1;
        return;
    }
...
void do_dir(void)
{
    ...
    while (peek_byte(cursor) != 0) {
        if (peek_byte(cursor) == '/') {
            ...
            if (!next_token(pattern, sizeof(pattern))) {
                break;
            }
            have_pat = 1;
            skip_spaces();
    ...
    while (1) {
        if (opt_o) {
            ...
            if (opt_w) {
                print_string(buf_addr(dir_dta, 0x1e));
                namelen = str_len(buf_addr(dir_dta, 0x1e));
                while (namelen < 13) {
                    print_char(' ');
                    namelen = namelen + 1;
                }
                ...
                print_string(buf_addr(dir_dta, 0x1e));
                i = str_len(buf_addr(dir_dta, 0x1e));
                while (i < 13) {
                    print_char(' ');
                    i = i + 1;
                }
                if (attr & FA_DIRENT) {
                    ...
                    size = peek_word(buf_addr(dir_dta, 0x1A));
                    size_hi = peek_word(buf_addr(dir_dta, 0x1C));
            ...
            if (!(buf_get(dir_dta, 0x1e) == '.' && (buf_get(dir_dta, 0x1f) == 0 || (buf_get(dir_dta, 0x1f) == '.' && buf_get(dir_dta, 0x20) == 0)))) {
    ...
    print_dollar(" bytes\r\n                    $");
    {
        ...
        if (dos_disk_free(0, &df) == 0) {
            ...
            size = df.free_clusters * df.secs_per_clust * df.bytes_per_sect;
            if (size < 0) {
                size = 0;
            }
            print_num(size);
...
void do_date(void)
{
    ...
    if (!next_token(arg1, sizeof(arg1))) return;
    parse_pos = 0;
    month = parse_uint(arg1);
    if (buf_get(arg1, parse_pos) == '-' || buf_get(arg1, parse_pos) == '/') parse_pos = parse_pos + 1;
    day = parse_uint(arg1);
...
void do_vol(void)
{
    ...
    print_char(drive + 'A');
    if (dos_find_first(pattern, FA_LABEL) == -1) {
...
void do_for(void)
{
    ...
    if (!next_token(arg1, sizeof(arg1))) {
        ...
        buf_set(for_var, 1, 0);
        if (!next_token(arg1, sizeof(arg1)) || !str_eq(arg1, "IN")) {
            last_errorlevel = 1;
            print_dollar("FOR %v IN (set) DO cmd\r\n$");
        } else {
            skip_spaces();
            if (peek_byte(cursor) != '(') {
                ...
                skip_spaces();
                if (!next_token(arg1, sizeof(arg1)) || !str_eq(arg1, "DO")) {
                    last_errorlevel = 1;
                    print_dollar("FOR %v IN (set) DO cmd\r\n$");
                } else {
                    skip_spaces();
                    ...
                    if (wild) {
                        dos_set_dta(dta);
                        if (dos_find_first(for_item, FA_DIRENT) != -1) {
                            while (1) {
...
void if_run_tail(int take_then)
{
    ...
    if (take_then) {
        if (buf_get(if_then, 0) != 0) {
            str_copy(cmd, if_then, sizeof(cmd));
            dispatch();
        }
    } else {
        if (buf_get(if_else, 0) != 0) {
            str_copy(cmd, if_else, sizeof(cmd));
            dispatch();
        }
    }
}
...
void do_if(void)
{
    ...
    if (eq < 0) {
        str_copy(left, arg1, sizeof(left));
        if (!next_token(arg1, sizeof(arg1))) {
            return;
        }
        if (!str_eq(arg1, "==")) {
            ...
            if (!next_token(right, sizeof(right))) {
                return;
            }
        }
    } else {
...
int setup_redirection(void)
{
    ...
    if (redir_out_kind != REDIR_NONE) {
        if (redir_out_kind == REDIR_APPEND) {
...
void parse_redirection(void)
{
    ...
    int n;
    int kind;
    redir_in_kind = REDIR_NONE;
    redir_out_kind = REDIR_NONE;
    redir_in_handle = -1;
    redir_out_handle = -1;
    saved_stdin = -1;
    saved_stdout = -1;
...
void dispatch_plain(void)
{
    ...
    if (!next_token(prog, sizeof(prog))) return;
    ...
    if (str_eq(prog, "MD") || str_eq(prog, "MKDIR")) {
        if (!next_token(arg1, sizeof(arg1)) || dos_mkdir(arg1) == -1) {
    ...
    if (str_eq(prog, "RD") || str_eq(prog, "RMDIR")) {
        if (!next_token(arg1, sizeof(arg1)) || dos_rmdir(arg1) == -1) {
    ...
    if (str_eq(prog, "REN") || str_eq(prog, "RENAME")) {
        if (!next_token(arg1, sizeof(arg1)) || !next_token(arg2, sizeof(arg2)) || dos_rename(arg1, arg2) == -1) {
    ...
    if (str_eq(prog, "ECHO") || is_echo_punct(prog)) {
        ...
        skip_spaces();
        echo_save = cursor;
        if (next_token(arg1, sizeof(arg1)) && peek_byte(cursor) == 0) {
            if (str_eq(arg1, "ON")) {
                echo_on = 1;
                return;
            }
    ...
    str_copy(prog_base, prog, sizeof(prog_base));
    make_exec_tail();
    if (!try_exec_name()) {
        str_copy(prog, prog_base, sizeof(prog));
        ...
        if (!str_has(prog_base, '\\') && !str_has(prog_base, '/')) {
            ...
            if (env_copy_var("PATH", env_pathbuf, 128)) {
                ...
                while (1) {
                    ...
                    if (c > 0) {
                        ...
                        str_copy(prog, env_join, sizeof(prog));
            ...
            str_copy(prog, "A:\\BIN\\", sizeof(prog));
...
int run_pipe(void)
{
    ...
    while (1) {
        ...
        dos_force_dup(h, 0);
        dos_close(h);
        str_copy(pipe_input, pipe_tmp, sizeof(pipe_input));
        input_active = 1;
        str_copy(cmd, pipe_rhs, sizeof(cmd));
    }
}
