...
/* Returns 0 on success, -1 on failure. Fills *out when successful. */
int dos_disk_free(int drive, struct DiskFree *out)
{
    ...
    out->free_clusters = df_ax;
    out->secs_per_clust = df_bx;
    out->bytes_per_sect = df_cx;
    out->total_clusters = df_dx;
    return 0;
}
