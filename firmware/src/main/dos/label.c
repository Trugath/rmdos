...
int main(void)
{
    ...
    have_label = 0;
    args_init();
    if (args_token(tok, sizeof(tok))) {
        if (is_drive(tok)) {
            drive = toupper_ch(buf_get(tok, 0)) - 'A';
            if (args_token(label, sizeof(label))) {
                have_label = 1;
            }
        } else {
            copy_name(label, tok);
    ...
    if (!have_label) {
        dos_set_dta(dta);
        if (dos_find_first(search, FA_LABEL) == -1) {
