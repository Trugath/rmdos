        ...
        if (!(attr & FA_DIRENT)) {
            if (!opt_a || (attr & 0x20)) {
                ...
                if (destination_is_dir) {
                    ...
                    str_copy_limit(destination_path, dst, sizeof(destination_path));
                }
...
static int walk_tree(int depth)
{
    ...
    dos_set_dta(dta);
    if (dos_find_first(search_pattern, FA_ALL) == -1) return 1;
    do {
        attr = buf_get(dta, 0x15);
        if (attr & FA_DIRENT) {
            copy_entry_name(dta);
...
static int prepare_source(void)
{
    ...
    if (path_ends_separator(root)) {
        ...
        if (attr != -1 && (attr & FA_DIRENT)) source_is_dir = 1;
    }
    if (source_is_dir) {
        str_copy_limit(file_pattern, "*.*", sizeof(file_pattern));
        source_has_wild = 1;
        return 1;
    }
    i = 0;
...
int main(void)
{
    ...
    while (args_skip()) {
        ...
        if (c == '/' || c == '-') {
            ...
            args_token(src, sizeof(src));
        } else if (buf_get(dst, 0) == 0) {
            args_token(dst, sizeof(dst));
        } else {
            print_dollar(msg_u);
            return 1;
        }
    ...
    if (path_ends_separator(root_dst)) {
        ...
        if (attr != -1 && (attr & FA_DIRENT)) destination_is_dir = 1;
    }
