"""E2E: DHCP.COM lease from k8086 virtual NAT DHCP server."""
...
def test_dhcp_on_image() -> None:
    raw = IMAGE.read_bytes()
    ent = fat12.find_directory_entry(raw, "BIN\\DHCP.COM")
    assert ent.size_bytes > 0
    ae = fat12.find_directory_entry(raw, "AUTOEXEC.BAT")
    assert ae.size_bytes >= 4


def test_dhcp_e2e() -> None:
    env = os.environ.copy()
        ...
        while time.time() < deadline:
            if SERIAL.is_file():
                ...
                if (
                    ...
                    and "10.0.2." in text
                    and "255.255.255.0" in text
                    and "10.0.2.2" in text
                    and "DHCP failed" not in text
                ):
                    ok = True
                    break
                if "DHCP failed" in text or "NIC init failed" in text:
                    break
    ...
    if not ok:
        raise AssertionError(f"DHCP lease output not seen.\n---\n{text}\n---")
    print("test_dhcp_e2e: OK")
